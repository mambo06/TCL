    ...
    for i, (x, label) in train_tqdm:
        if  mode == 'train' :
            ...
            if x.shape[0] == config['batch_size'] :
                x_tilde_list = model.subset_generator(torch.cat(
                    (x[:int(config['batch_size']/2)], x_[int(config['batch_size']/2):] ),
                    0
                    )
                )
                x_tilde_list += model.subset_generator(torch.cat(
                    (x_[:int(config['batch_size']/2)],x[int(config['batch_size']/2):]),
                    0
                    )
                )
            else :
        ...
        for xi in x_tilde_list:
            ...
            # Extract latent
            _, latent, _ = encoder(Xbatch) # encode
            # Collect latent
        ...
        if  mode != 'train' :
            ...
            for i in range(config['n_subsets']):
                if x.shape[0] == config['batch_size']:
                    latent_list_.append( torch.cat(
                        (latent_list[i][:int(config['batch_size']/2)] , 
                            latent_list[i+config['n_subsets']][int(config['batch_size']/2):]),
                        0)
                        )
                else:
                    latent_list_.append(latent_list[i][:x.shape[0]])
